...
class HeavyPart
{
    alignas(64) Bucket_ES buckets[bucket_num];
public:
    HeavyPart();
    ~HeavyPart();
    ...
    int insert_precomp_hashes(uint8_t *key, uint8_t *swap_key, uint32_t &swap_val, uint32_t &pos, int &f);
...
{
    memset(buckets, 0, sizeof(Bucket_ES) * bucket_num);
}
...
template<int bucket_num>
inline int HeavyPart<bucket_num>::query(uint8_t *key)
{
    uint32_t fp;
    int pos = CalculateFP(key, fp);
...
{
    return bucket_num * sizeof(Bucket_ES);
}
...
template<int bucket_num>
inline int HeavyPart<bucket_num>::insert_precomp_hashes(uint8_t *key, uint8_t *swap_key, uint32_t &swap_val, uint32_t &post, int &f)
{
	int pos = int(post);
	uint32_t fp = *((uint32_t*)key);
	//int pos = CalculateFP(key, fp); // Pos precomputed
	uint32_t min_counter_val;
	int min_counter;
#ifdef USING_SIMD_ACCELERATION
    do{
