...
public:
    ...
    void insert(uint8_t *key, int f, uint32_t &post);
	void swap_insert(uint8_t *key, int f);
	void swap_insert(uint8_t *key, int f, uint32_t &post);
...
// Precomputed hashes
template<int init_mem_in_bytes>
void LightPart<init_mem_in_bytes>::insert(uint8_t *key, int f, uint32_t &post)
{
    //uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, KEY_LENGTH_4);
    uint32_t pos = post;// hash_val % (uint32_t)counter_num;
...
template<int init_mem_in_bytes>
void LightPart<init_mem_in_bytes>::swap_insert(uint8_t *key, int f, uint32_t &post)
{
    //uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, KEY_LENGTH_4);
    uint32_t pos = post;//hash_val % (uint32_t)counter_num;
